within Exergy.XBuildings.Examples.Book.AirConditioning;
model case1_varAirFlow_improve_performance_opt
 ...
 // replaceable package MediumW = Buildings.Media.Water "Medium for water";
  ...
  parameter Real eps = 0.1 "Heat recovery effectiveness";
  ...
  parameter Modelica.SIunits.Temperature TOut_nominal = 273.15+43
  ...
  parameter Modelica.SIunits.Temperature TWSup_nominal = 273.15+7
  ...
  parameter Modelica.SIunits.MassFlowRate mW_flow_nominal=
    -1*QCoiC_flow_nominal/(TWRet_nominal-TWSup_nominal)/4200
  ...
  Exergy.XBuildings.Fluid.Movers.FlowControlled_m_flow fan(
    ...
    show_T=true) "Supply air fan"
    annotation (Placement(transformation(extent={{40,-30},{60,-10}})));
 // Exergy.XBuildings.Fluid.HeatExchangers.ConstantEffectiveness hex(
  Exergy.XBuildings.Fluid.MassExchangers.ConstantEffectiveness hex(
  ...
  Exergy.XBuildings.Fluid.HeatExchangers.WetCoilCounterFlow cooCoi(
    ...
    m2_flow_nominal=mA_flow_nominal,
    dp1_nominal=6000,
    UA_nominal=-U_k*QCoiC_flow_nominal/
   ...
   // energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyStateInitial)
    ...
    Exergy.XBuildings.Fluid.Sources.FixedBoundary sinWat(          redeclare
      package Medium =
               MediumW,
    nPorts=1,
    T=TWSup_nominal) "Sink for water circuit"
  ...
  Modelica.Blocks.Sources.Constant TRooSetPoi(k=5) "Room temperature set point"
    annotation (Placement(transformation(extent={{-142,-134},{-126,-118}})));
  ...
  Utilities.ViewObject viewObject(nEnergy={0,1,0,0})
    ...
    Real productExergyEff=(vol.viewObject.Ex_flow_loss+theCon.viewObject.Ex_flow_loss)/max(Modelica.Constants.eps,viewObject.Ex_flow_loss);
  Buildings.Controls.Continuous.LimPID heaCoiCon(
    ...
    Td=Td,
    yMin=0.1,
    yMax=3*mW_flow_nominal) "Controller for heating coil"
    annotation (Placement(transformation(extent={{-104,-106},{-84,-86}})));
   // k=2,
   // Ti=100,
  Fluid.Movers.PowerSourceForMovers powerSourceForMovers
    ...
    /*************************************************/
  //ExternData.XMLFile xmlfile(fileName=Modelica.Utilities.Files.loadResource("modelica://Exergy/parameter.xml"));
  ExternData.XMLFile xmlfile(fileName=Modelica.Utilities.Files.loadResource("parameter.xml"));
  parameter String resultName = "result.txt";
  parameter String inputFileName = "modelicaSchedule.txt"
    "File on which data is present";

  parameter Real K(start=1,fixed=false, unit="1");
  parameter Real Ti(start=1,fixed=false, unit="1");
    parameter Real Td(start=1,fixed=false, unit="1");

      parameter Real K_A(start=1,fixed=false, unit="1");
  parameter Real Ti_A(start=1,fixed=false, unit="1");
    parameter Real Td_A(start=1,fixed=false, unit="1");
    parameter Real U_k(start=2.2,fixed=false, unit="1");
//  parameter Modelica.SIunits.Temperature      roomSetT=273.15+22;
 // Real objectFuctionValue(start=0,fixed=true);
 Real totalExergyLoss;
 Real objectFuctionValue;
  Buildings.Fluid.Sensors.TemperatureTwoPort senTem(redeclare package Medium =
        MediumW, m_flow_nominal=mW_flow_nominal)
    annotation (Placement(transformation(extent={{-66,-50},{-50,-70}})));
  Modelica.Blocks.Math.Feedback feedback
    annotation (Placement(transformation(extent={{-152,-106},{-132,-86}})));
  Buildings.Fluid.Sensors.TemperatureTwoPort senTem1(redeclare package Medium
      = MediumW, m_flow_nominal=mW_flow_nominal) annotation (Placement(
        transformation(
        extent={{-8,10},{8,-10}},
        rotation=-90,
        origin={-4,-66})));
  Buildings.Controls.Continuous.LimPID heaCoiCon1(
    initType=Modelica.Blocks.Types.InitPID.InitialState,
    k=K_A,
    Ti=Ti_A,
    controllerType=Modelica.Blocks.Types.SimpleController.PID,
    Td=Td_A,
    yMin=0.03,
    yMax=mA_flow_nominal) "Controller for heating coil"
    annotation (Placement(transformation(extent={{-28,2},{-12,18}})));
  Modelica.Blocks.Sources.Constant TRooSetPoi1(
                                              k=TRooSet)
    "Room temperature set point"
    annotation (Placement(transformation(extent={{-50,-12},{-36,2}})));
initial equation
  //K=1;
  // der(objectFuctionValue)=viewObject.Ex_flow_loss;
 K =xmlfile.getReal("PID.K");
 Ti =xmlfile.getReal("PID.Ti");
 Td =xmlfile.getReal("PID.Td");
  K_A =xmlfile.getReal("PID.K_A");
 Ti_A =xmlfile.getReal("PID.Ti_A");
 Td_A =xmlfile.getReal("PID.Td_A");
  U_k =xmlfile.getReal("U_k");
equation
      //  objectFuctionValue=K*K+Ti*(K-Ti)+Ti*Ti;

    der(totalExergyLoss)=viewObject.Ex_flow_loss;

   when terminal() then
      objectFuctionValue=2000*(cooCoi.UA_nominal/50)^0.6*0.04/(1-(1+0.04)^(-15))+(totalExergyLoss/1e6)*40*0.1;
    Modelica.Utilities.Streams.print("PID.K = " +
    realString(number=heaCoiCon.k, minimumWidth=1, precision=16), resultName);
    Modelica.Utilities.Streams.print("PID.Ti = " +
    realString(number=heaCoiCon.Ti, minimumWidth=1, precision=16), resultName);
        Modelica.Utilities.Streams.print("PID.Td = " +
    realString(number=heaCoiCon.Td, minimumWidth=1, precision=16), resultName);

        Modelica.Utilities.Streams.print("PID.K_A = " +
    realString(number=heaCoiCon1.k, minimumWidth=1, precision=16), resultName);
    Modelica.Utilities.Streams.print("PID.Ti_A = " +
    realString(number=heaCoiCon1.Ti, minimumWidth=1, precision=16), resultName);
        Modelica.Utilities.Streams.print("PID.Td_A = " +
    realString(number=heaCoiCon1.Td, minimumWidth=1, precision=16), resultName);

            Modelica.Utilities.Streams.print("U_k = " +
    realString(number=U_k, minimumWidth=1, precision=16), resultName);
                Modelica.Utilities.Streams.print("cooCoi.UA_nominal = " +
    realString(number=cooCoi.UA_nominal, minimumWidth=1, precision=16), resultName);

    Modelica.Utilities.Streams.print("objectFuctionValue = " +
    realString(number=objectFuctionValue, minimumWidth=1, precision=16), resultName);
   end when;
  ...
  connect(myWea.weaBus, out.weaBus) annotation (Line(
      points={{-161.2,45.8},{-142,45.8},{-142,6},{-142,-21.8},{-140,-21.8}},
      color={255,204,51},
      thickness=0.5));
  ...
  connect(viewObject.viewOutput, viewRoute.viewTotal) annotation (Line(points={{
      ...
      pattern=LinePattern.Dash));
  connect(hex.viewOutput, viewRoute.viewOutput[2]) annotation (Line(points={{-88.4,
          -16.2},{-96,-16.2},{-96,94.38},{101.91,94.38}}, color={28,108,200},
      pattern=LinePattern.Dash));
  connect(sinWat.viewOutput, viewRoute.viewOutput[3]) annotation (Line(points={{-76.4,
          -56.2},{-76.4,24},{-76.4,102},{101.91,102},{101.91,94.98}},
        color={28,108,200},
      pattern=LinePattern.Dash));
  ...
  connect(theCon.viewOutput, viewRoute.viewOutput[10]) annotation (Line(points={{39.6,
        ...
        color={28,108,200},
      pattern=LinePattern.Dash));
  connect(heaCoiCon.y, souWat.m_flow_in) annotation (Line(points={{-83,-96},{-83,
          -92},{-40,-92}},            color={0,0,127},
  ...
  connect(powerSourceForMovers.viewPort, viewRoute.viewOutput[11]) annotation (
      Line(points={{45.72,5.52},{96,5.52},{96,88},{96,99.78},{101.91,99.78}},
        color={28,108,200},
      pattern=LinePattern.Dash));
  connect(senTem.port_a, sinWat.ports[1]) annotation (Line(points={{-66,-60},{-76,
          -60},{-76,-66}}, color={0,127,255}));
  connect(senTem.port_b, cooCoi.port_b1) annotation (Line(points={{-50,-60},{-48,
          -60},{-48,-58},{-48,-32},{-40,-32}}, color={0,127,255}));
  connect(souWat.ports[1], senTem1.port_b) annotation (Line(points={{-20,-100},{
          -4,-100},{-4,-74}}, color={0,127,255}));
  connect(senTem1.port_a, cooCoi.port_a1)
    annotation (Line(points={{-4,-58},{-4,-32},{-20,-32}}, color={0,127,255}));
  connect(senTem.T, feedback.u1) annotation (Line(
      points={{-58,-71},{-62,-71},{-62,-78},{-150,-78},{-150,-96}},
      color={0,0,127},
      pattern=LinePattern.Dot));
  connect(senTem1.T, feedback.u2) annotation (Line(
      points={{-15,-66},{-12,-66},{-12,-112},{-142,-112},{-142,-104}},
      color={0,0,127},
      pattern=LinePattern.Dot));
  connect(TRooSetPoi1.y, heaCoiCon1.u_m) annotation (Line(points={{-35.3,-5},{-24,
          -5},{-20,-5},{-20,0.4}}, color={0,0,127}));
  connect(senTemRoo.T, heaCoiCon1.u_s) annotation (Line(
      points={{90,80},{90,80},{90,40},{90,80},{-29.6,80},{-29.6,10}},
      color={0,0,127},
      pattern=LinePattern.Dot));
  connect(feedback.y, heaCoiCon.u_s) annotation (Line(points={{-133,-96},{-106,-96}},
                       color={0,0,127}));
  connect(TRooSetPoi.y, heaCoiCon.u_m) annotation (Line(points={{-125.2,-126},{-114,
          -126},{-114,-128},{-94,-128},{-94,-108}}, color={0,0,127}));
  connect(heaCoiCon1.y, fan.m_flow_in) annotation (Line(points={{-11.2,10},{8,10},
          {8,14},{49.8,14},{49.8,-8}}, color={0,0,127}));
...
First implementation.
    ...
    Diagram(coordinateSystem(preserveAspectRatio=false,extent={{-180,-160},{140,
            120}})),
...
end case1_varAirFlow_improve_performance_opt;
