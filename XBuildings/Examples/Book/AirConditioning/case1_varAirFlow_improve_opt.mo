...
model case1_varAirFlow_improve_opt
 ...
 // Exergy.XBuildings.Fluid.HeatExchangers.ConstantEffectiveness hex(
  ...
  Exergy.XBuildings.Fluid.HeatExchangers.WetCoilCounterFlow cooCoi(
    ...
    m2_flow_nominal=mA_flow_nominal,
    dp1_nominal=6000,
    UA_nominal=-2.2*QCoiC_flow_nominal/
...
equation
   ...
   when terminal() then
    ...
    realString(number=objectFuctionValue/1e6, minimumWidth=1, precision=16), resultName);
   end when;
...
end case1_varAirFlow_improve_opt;
