...
model System3pid
 ...
 // replaceable package MediumW = Buildings.Media.Water "Medium for water";
  ...
  parameter Modelica.SIunits.Temperature TASup_nominal = 273.15+60
    "Nominal air temperature supplied to room";
  parameter Modelica.SIunits.Temperature TRooSet = 273.15+24
    "Nominal room air temperature";
  parameter Modelica.SIunits.Temperature TOut_nominal = 273.15+0
    "Design outlet air temperature";
  parameter Modelica.SIunits.Temperature THeaRecLvg=TOut_nominal - 0.1*(TOut_nominal-TRooSet)
  ...
  parameter Modelica.SIunits.HeatFlowRate QRooInt_flow=
     0 "Internal heat gains of the room";
  parameter Modelica.SIunits.HeatFlowRate QRooC_flow_nominal=
    -QRooInt_flow-5*10E3/30*(TOut_nominal-TRooSet)
  ...
  parameter Modelica.SIunits.Temperature TWSup_nominal = 273.15+40
    "Water supply temperature";
  parameter Modelica.SIunits.Temperature TWRet_nominal = 273.15+36
  ...
  Utilities.ViewRoute viewRoute(nSubSystem=10)
  ...
  Buildings.Controls.Continuous.LimPID heaCoiCon(
    ...
    annotation (Placement(transformation(extent={{-88,-112},{-68,-92}})));
equation
  ...
  connect(myWea.weaBus, weaBus) annotation (Line(
      points={{-148,46},{-130,46},{-130,50},{-110,50}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%second",
      index=1,
  ...
  connect(myWea.weaBus, out.weaBus) annotation (Line(
      points={{-148,46},{-142,46},{-142,6},{-142,-21.8},{-140,-21.8}},
      color={255,204,51},
      thickness=0.5));
  connect(out.viewOutput, viewRoute.viewOutput[1]) annotation (Line(points={{-120.4,
          -12.2},{-120.4,60},{-120.4,93.81},{101.91,93.81}},
                                                         color={28,108,200}));
  ...
  connect(hex.viewOutput, viewRoute.viewOutput[2]) annotation (Line(points={{-90.4,
          -16.2},{-96,-16.2},{-96,94.47},{101.91,94.47}}, color={28,108,200}));
  connect(sinWat.viewOutput, viewRoute.viewOutput[3]) annotation (Line(points={{
          -60.4,-56.2},{-60.4,24},{-60.4,102},{101.91,102},{101.91,95.13}},
        color={28,108,200}));
  connect(cooCoi.viewOutput, viewRoute.viewOutput[4]) annotation (Line(points={{
          -39.6,-35.8},{-39.6,-62},{-39.6,-72},{110,-72},{110,82},{101.91,82},{101.91,
          95.79}}, color={28,108,200}));
  connect(TOut.viewOutput, viewRoute.viewOutput[5]) annotation (Line(points={{-0.4,
          59.8},{-0.4,82},{-0.4,96.45},{101.91,96.45}}, color={28,108,200}));
  connect(preHea.viewOutput, viewRoute.viewOutput[6]) annotation (Line(points={{
          29.6,81.8},{29.6,83.8},{101.91,83.8},{101.91,97.11}}, color={28,108,200}));
  connect(vol.viewOutput, viewRoute.viewOutput[7]) annotation (Line(points={{79.6,
          39.8},{101.91,39.8},{101.91,97.77}}, color={28,108,200}));
  connect(fan.viewOutput, viewRoute.viewOutput[8]) annotation (Line(points={{59.6,
          -10.2},{94,-10.2},{108,-10.2},{108,98.43},{101.91,98.43}}, color={28,108,
          200}));
  connect(souWat.viewOutput, viewRoute.viewOutput[9]) annotation (Line(points={{
          -20.4,-90.2},{101.91,-90.2},{101.91,99.09}}, color={28,108,200}));
  connect(theCon.viewOutput, viewRoute.viewOutput[10]) annotation (Line(points={
          {39.6,59.8},{62,59.8},{62,68},{60,68},{60,99.75},{101.91,99.75}},
        color={28,108,200}));
  connect(heaCoiCon.y, souWat.m_flow_in) annotation (Line(points={{-67,-102},{-58,
          -102},{-58,-100},{-40,-100},{-40,-92}}, color={0,0,127}));
  connect(TRooSetPoi.y, heaCoiCon.u_s) annotation (Line(points={{-149,-80},{-124,
          -80},{-124,-94},{-90,-94},{-90,-102}}, color={0,0,127}));
  connect(senTemRoo.T, heaCoiCon.u_m) annotation (Line(points={{90,80},{84,80},{
          84,-140},{-82,-140},{-82,-114},{-78,-114}}, color={0,0,127}));
...
First implementation.
    ...
    __Dymola_Commands(file=
     "modelica://Buildings/Resources/Scripts/Dymola/Examples/Tutorial/SpaceCooling/System3.mos"
        "Simulate and plot"),
    experiment(StartTime=0, StopTime=60));
end System3pid;
