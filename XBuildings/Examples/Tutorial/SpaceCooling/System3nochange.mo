...
model System3nochange
 ...
 Exergy.XBuildings.Fluid.MixingVolumes.MixingVolume vol(
    ...
    mSenFac=3) annotation (Placement(transformation(extent={{60,20},{80,40}})));
 ...
 // Buildings.Fluid.Sources.FixedBoundary sinWat(nPorts=1, redeclare package
   //   Medium = MediumW) "Sink for water circuit"
  ...
  parameter Boolean exChangingWithEnv = true;

 // inner Utilities.RefEnv refEnv
  XModelica.Thermal.HeatTransfer.Components.HeatCapacitor_Di heatCapacitor(C=100)
    annotation (Placement(transformation(extent={{104,14},{124,34}})));
  //(T=out.medium.state.T)
  //,p=out.medium.state.p,vaporFra=out.medium.state.X[1])
  inner Utilities.RefEnv refEnv;

  Utilities.ViewRoute viewRoute(nSubSystem=11)
    annotation (Placement(transformation(extent={{-18,82},{2,102}})));
...
equation
  ...
  connect(mWat_flow.y, souWat.m_flow_in) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(theCon.viewOutput, viewRoute.viewOutput[1]) annotation (Line(points={{39.6,
          59.8},{39.6,70},{-30,70},{-30,89.0727},{-18.1,89.0727}},color={28,108,
          200}));
  connect(vol.viewOutput, viewRoute.viewOutput[2]) annotation (Line(points={{79.6,
          39.8},{79.6,62},{-22,62},{-22,89.6182},{-18.1,89.6182}},
                                                             color={28,108,200}));
  connect(viewRoute.viewTotal, viewPort) annotation (Line(points={{1.8,91.9},{56,
          91.9},{56,94},{118,94}}, color={28,108,200}));
  connect(TOut.viewOutput, viewRoute.viewOutput[3]) annotation (Line(points={{-0.4,
          59.8},{8,59.8},{8,80},{-18.1,80},{-18.1,90.1636}},
                                                          color={28,108,200}));
  connect(preHea.viewOutput, viewRoute.viewOutput[4]) annotation (Line(points={{39.6,
          89.8},{39.6,104},{-18.1,104},{-18.1,90.7091}},    color={28,108,200}));
  connect(souWat.viewOutput, viewRoute.viewOutput[5]) annotation (
      Line(points={{-20.4,-90.2},{-16,-90.2},{-16,20},{-56,20},{-56,
          91.2545},{-18.1,91.2545}},
                               color={28,108,200}));
  connect(sinWat.viewOutput, viewRoute.viewOutput[6]) annotation (Line(points={{-60.4,
          -56.2},{-46,-56.2},{-46,91.8},{-18.1,91.8}},       color={28,108,200}));
  connect(out.viewOutput, viewRoute.viewOutput[7]) annotation (Line(points={{-120.4,
          -12.2},{-120,-12.2},{-120,92.3455},{-18.1,92.3455}}, color={28,108,200}));
  connect(hex.viewOutput, viewRoute.viewOutput[8]) annotation (Line(points={{-90.4,
          -16.2},{-76,-16.2},{-76,102},{-18.1,102},{-18.1,92.8909}},color={28,108,
          200}));
  connect(fan.viewOutput, viewRoute.viewOutput[9])  annotation (Line(
        points={{59.6,-10.2},{-36,-10.2},{-36,90},{-18.1,90},{-18.1,
          93.4364}},
                  color={28,108,200}));
  connect(heatCapacitor.port, vol.heatPort) annotation (Line(points={{114,14},{114,
          8},{60,8},{60,30}}, color={191,0,0}));
  connect(heatCapacitor.viewOutput, viewRoute.viewOutput[10]) annotation (Line(
        points={{123.6,33.8},{152,33.8},{152,93.9818},{-18.1,93.9818}},
                                                                  color={28,108,
          200}));
  connect(cooCoi.viewOutput, viewRoute.viewOutput[11]) annotation (Line(points={{-39.6,
          -35.8},{-62,-35.8},{-62,94.5273},{-18.1,94.5273}},        color={28,108,
          200}));
    annotation (Placement(transformation(extent={{-164,76},{-144,96}})),
...
First implementation.
    ...
    Diagram(coordinateSystem(preserveAspectRatio=false,extent={{-180,-140},{120,
            100}})),
