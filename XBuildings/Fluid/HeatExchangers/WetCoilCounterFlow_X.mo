...
model WetCoilCounterFlow_X
  ...
  extends Buildings.Fluid.HeatExchangers.DryCoilCounterFlow(
      ...
      Exergy.XBuildings.Fluid.HeatExchangers.BaseClasses.HexElementLatent_X
                                                                         ele[nEle]);
 //     Buildings.Fluid.HeatExchangers.BaseClasses.HexElementLatent ele[nEle]);
  ...
  Modelica.SIunits.MassFlowRate mWat_flow "Water flow rate";
   ...
   Modelica.SIunits.MassFlowRate mWat_ele_flow[nEle] "Water flow rate";
   Modelica.SIunits.Temperature mWat_ele_T[nEle];
   Modelica.SIunits.Pressure mWat_P=port_b2.p;
...
equation
  ...
  connect(ele.viewPort, viewRoute.viewOutput[1:4]) annotation (Line(
        points={{20,19.8},{42,19.8},{42,20},{61.9,20},{61.9,94.05}},
        color={28,108,200}));
 annotation (
