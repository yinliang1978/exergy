...
/*Working fluid*/
  ...
  parameter Modelica.SIunits.Time TT_hot=1
    ...
    annotation (Dialog(enable=filter_dMdt, tab="Numerical options"));
/******************************* COMPONENTS ***********************************/
  Exergy.XThermoCycle.Components.FluidFlow.Pipes.Flow1Dim Hotside(
    ...
    Discretization=Discretization) annotation (Placement(
        transformation(extent={{38,134},{-36,74}})));
  ...
  Exergy.XThermoCycle.Components.FluidFlow.Pipes.Flow1Dim Coldside(
    ...
    Unom_v=Unom_v_cold,
    Discretization=Discretization) annotation (Placement(
        transformation(extent={{-44,-158},{40,-74}})));
