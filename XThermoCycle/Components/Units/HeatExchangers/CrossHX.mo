...
/* Working Fluid */
  ...
  Exergy.XThermoCycle.Components.FluidFlow.Pipes.MultiFlow1D multiFlow1D_DP(
    ...
    t_init=t_init_sf,
    A_f=A_sf,
    A=Athroat_sf) annotation (Placement(transformation(extent={{-20,-66},
            {20,-32}})));
Exergy.XThermoCycle.Components.HeatFlow.Walls.MetalWallL metalWallCell[N](
    each M_wall=M_wall_tot/N,
    each c_wall=c_wall,
    each Tstart_wall=T_start_wall,
    each steadystate_T_wall=steadystate_T_wall,
    each Aext=A_sf/N,
    each Aint=A_wf/N) annotation (Placement(transformation(extent={{-34,
            -18},{34,30}})));
    Exergy.XThermoCycle.Components.FluidFlow.Pipes.Cell1Dim flow1DimCell[N](
    redeclare package Medium = Medium1,
    redeclare final model HeatTransfer = Medium1HeatTransferModel,
    each Vi=V_wf/N,
    each Ai=A_wf/N,
    ...
    each hstart=hstart_wf_in)
    annotation (Placement(transformation(extent={{-28,74},{26,30}})));
