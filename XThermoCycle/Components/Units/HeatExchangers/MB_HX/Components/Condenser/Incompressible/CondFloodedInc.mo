within Exergy.XThermoCycle.Components.Units.HeatExchangers.MB_HX.Components.Condenser.Incompressible;
model CondFloodedInc
...
parameter Boolean eps_NTU = false "Set to true for eps-NTU heat transfer" annotation (Dialog(group = "Heat transfer"));
  ...
  Interfaces.MbOut mbOut[nCV]
    annotation (Placement(transformation(extent={{-22,36},{20,54}})));
equation
...
equation
  ...
  connect(volumeSC.outFlow,OutFlow)  annotation (Line(
      points={{48,2.1},{68,2.1},{68,4},{80,4}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(volumeTP.mbOut, mbOut[1]) annotation (Line(
      points={{-32,11},{-32,36},{-11.5,36},{-11.5,45}},
      color={0,0,0},
      smooth=Smooth.None));
  connect(volumeSC.mbOut, mbOut[2]) annotation (Line(
      points={{38,11},{38,36},{9.5,36},{9.5,45}},
      color={0,0,0},
      smooth=Smooth.None));
    annotation (Diagram(coordinateSystem(preserveAspectRatio=false,extent={{-100,
            -20},{80,40}}),
                        graphics),Icon(coordinateSystem(extent={{-100,-20},{80,40}},
                   preserveAspectRatio=true),
                                     graphics={
                               Bitmap(extent={{-102,106},{80,-88}}, fileName="modelica://ThermoCycle/Resources/Images/MB/Cond_F_wf.png")}));
end CondFloodedInc;
