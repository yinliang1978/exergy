within Exergy.XThermoCycle.Components.Units.HeatExchangers.MB_HX.Components.Condenser.Incompressible.Unit;
model CondDwInc
...
annotation (choicesAllMatching = true);
  ...
  Exergy.XThermoCycle.Components.Units.HeatExchangers.MB_HX.Components.Wall.wall
    ...
    n=2) annotation (Placement(transformation(extent={{-18,
            -24},{22,24}})));
  ThermoCycle.Interfaces.Fluid.Flange_Cdot InFlowSF
    annotation (Placement(transformation(extent={{70,48},{90,68}}),
        iconTransformation(extent={{70,48},{90,68}})));

    Incompressible.CondDryInc               CondDry(
    ...
    VoidF=VoidF)
    annotation (Placement(transformation(extent={{-26,-90},{36,-52}})));
...
connect( Wall.QmbOut, secondaryFluid.mbIn[nCV:-1:1]);
  ...
  connect(secondaryFluid.InFlow_sf, InFlowSF) annotation (Line(
      points={{26.8,80},{58,80},{58,58},{80,58}},
      color={255,0,0},
      smooth=Smooth.None));
...
public
  ...
  SummaryClass Summary(T_profile(Twf = CondDry.Summary.T_profile.T_cell[:],Tw= Wall.Summary.T_profile.T_cell[:], Tsf = secondaryFluid.Summary.T_profile.T_cell[:]),l_cell = CondDry.Summary.l_cell[:],Qwf = CondDry.Summary.Qtot,Qsf = secondaryFluid.Summary.Qtot);

equation
  connect(Wall.QmbIn, CondDry.mbOut) annotation (
      Line(
      points={{0.4,-21.6},{0.4,-36},{5,-36},{5,-53.9}},
      color={0,0,0},
      smooth=Smooth.None));

  connect(InflowPF, CondDry.InFlow) annotation (Line(
      points={{-100,-38},{-62,-38},{-62,-70.62},{-26.62,-70.62}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(CondDry.OutFlow, OutflowPF) annotation (Line(
      points={{35.38,-70.62},{60,-70.62},{60,-36},{80,-36}},
      color={0,0,255},
      smooth=Smooth.None));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,
            ...
            80}},  preserveAspectRatio=false),
                                     graphics={
                               Bitmap(extent={{-100,110},{80,-90}}, fileName="modelica://ThermoCycle/Resources/Images/MB/Cond_D.png")}));
end CondDwInc;
