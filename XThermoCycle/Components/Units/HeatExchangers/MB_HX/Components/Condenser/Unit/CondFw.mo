...
replaceable package Medium =
  ...
  ThermoCycle.Interfaces.Fluid.Flange_Cdot InFlowSF
    annotation (Placement(transformation(extent={{72,48},{92,68}}),
        iconTransformation(extent={{72,48},{92,68}})));
...
connect( Wall.QmbOut, secondaryFluid.mbIn[nCV:-1:1]);
  ...
  connect(secondaryFluid.InFlow_sf, InFlowSF) annotation (Line(
      points={{26.8,80},{54,80},{54,58},{82,58}},
      color={255,0,0},
      smooth=Smooth.None));
...
public
  ...
  SummaryClass Summary(T_profile(Twf = CondFlooded.Summary.T_profile.T_cell[:],Twall= Wall.Summary.T_profile.T_cell[:], Tsf = secondaryFluid.Summary.T_profile.T_cell[end:-1:1]),l_cell = CondFlooded.Summary.l_cell[:],Qwf = CondFlooded.Summary.Qtot,Qsf = secondaryFluid.Summary.Qtot);
...
equation
  ...
  connect(InflowPF, CondFlooded.InFlow) annotation (Line(
      points={{-98,-36},{-66,-36},{-66,-76},{-24.6,-76},{-24.6,-77.6}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(CondFlooded.OutFlow, OutflowPF) annotation (Line(
      points={{35.4,-77.6},{61.7,-77.6},{61.7,-40},{82,-40}},
      color={0,0,255},
      smooth=Smooth.None));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,
            ...
            80}},  preserveAspectRatio=false),
                                     graphics={
                               Bitmap(extent={{-100,110},{82,-92}}, fileName="modelica://ThermoCycle/Resources/Images/MB/Cond_F.png")}));
end CondFw;
