within Exergy.XThermoCycle.Components.Units.HeatExchangers.MB_HX.Components.Condenser.Unit;
model CondDw
...
annotation (choicesAllMatching = true);
  ...
  ThermoCycle.Interfaces.Fluid.FlangeB OutflowPF(redeclare package Medium = Medium)
    annotation (Placement(transformation(extent={{72,-48},{92,-28}}),
        iconTransformation(extent={{72,-48},{92,-28}})));
...
Exergy.XThermoCycle.Components.Units.HeatExchangers.MB_HX.Components.Wall.wall
    Wall(
    cp_w=cpw,
    L_total=Ltotal,
    M_w=Mw,
    TstartWall=TstartWall,
    n=2) annotation (Placement(transformation(extent={{-18,-24},
            {22,24}})));
  ThermoCycle.Interfaces.Fluid.Flange_Cdot InFlowSF
    ...
    Exergy.XThermoCycle.Components.Units.HeatExchangers.MB_HX.Components.Condenser.CondDry
    CondDry(
    ...
    VoidF=VoidF) annotation (Placement(transformation(extent=
            {{-30,-86},{32,-48}})));
...
connect( Wall.QmbOut, secondaryFluid.mbIn[nCV:-1:1]);
  ...
  connect(secondaryFluid.InFlow_sf, InFlowSF) annotation (Line(
      points={{26.8,80},{58,80},{58,60},{82,60}},
      color={255,0,0},
      smooth=Smooth.None));
...
if SteadyStatePF then
    der(CondDry.volumeSH.ll) = 0;
    der(CondDry.volumeTP.h_b) = 0;
  end if;
  if Set_h_pf_out then
    CondDry.volumeTP.h_b = h_pf_out;
  end if;
...
public
  ...
  SummaryClass Summary(T_profile(Twf = CondDry.Summary.T_profile.T_cell[:],Twall= Wall.Summary.T_profile.T_cell[:], Tsf = secondaryFluid.Summary.T_profile.T_cell[end:-1:1]),l_cell = CondDry.Summary.l_cell[:],Qwf = CondDry.Summary.Qtot,Qsf = secondaryFluid.Summary.Qtot);

equation
  connect(Wall.QmbIn, CondDry.mbOut) annotation (
      Line(
      points={{0.4,-21.6},{0.4,-36},{1,-36},{1,-49.9}},
      color={0,0,0},
      smooth=Smooth.None));

  connect(InflowPF, CondDry.InFlow) annotation (Line(
      points={{-98,-40},{-50,-40},{-50,-66.62},{-30.62,-66.62}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(CondDry.OutFlow, OutflowPF) annotation (Line(
      points={{31.38,-66.62},{66,-66.62},{66,-38},{82,-38}},
      color={0,0,255},
      smooth=Smooth.None));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,
            ...
            80}},  preserveAspectRatio=false),
                                     graphics={
                               Bitmap(extent={{-100,112},{82,-94}}, fileName="modelica://ThermoCycle/Resources/Images/MB/Cond_D.png")}));
end CondDw;
