within Exergy.XThermoCycle.Components.Units.HeatExchangers.MB_HX.Components.Condenser;
model CondDry
 ...
 /* Components */
  ...
  ThermoCycle.Interfaces.Fluid.FlangeB OutFlow(  redeclare final package Medium
    ...
    annotation (Placement(transformation(extent={{70,-2},{90,18}}),
        iconTransformation(extent={{70,-2},{90,18}})));
...
parameter Boolean eps_NTU = false "Set to true for eps-NTU heat transfer" annotation (Dialog(group = "Heat transfer"));
  ...
  Interfaces.MbOut mbOut[nCV]
    annotation (Placement(transformation(extent={{-16,30},{4,50}}),
        iconTransformation(extent={{-28,30},{16,50}})));
equation
...
equation
  ...
  connect(volumeTP.outFlow,OutFlow)  annotation (Line(
      points={{52,0.1},{60,0.1},{60,8},{80,8}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(volumeSH.mbOut, mbOut[1]) annotation (Line(
      points={{-26,11},{-26,46},{-6,46},{-6,35}},
      color={0,0,0},
      smooth=Smooth.None));
  connect(volumeTP.mbOut, mbOut[2]) annotation (Line(
      points={{42,9},{42,30},{20,30},{20,64},{-6,64},{-6,45}},
      color={0,0,0},
      smooth=Smooth.None));
