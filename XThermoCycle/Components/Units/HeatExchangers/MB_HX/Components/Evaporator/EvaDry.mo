within Exergy.XThermoCycle.Components.Units.HeatExchangers.MB_HX.Components.Evaporator;
model EvaDry
...
parameter Modelica.SIunits.Length lstartTP=1 "TP:Start value of length"
  ...
  Modelica.SIunits.Power qtot;
   Interfaces.MbOut   mbOut[nCV]
    annotation (Placement(transformation(extent={{-22,38},{20,54}})));
equation
...
equation
  ...
  connect(volumeSH.outFlow,OutFlow)
    annotation (Line(
      points={{60,0.1},{76,0.1},{76,2},{82,2}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(volumeTP.mbOut, mbOut[1])
    annotation (Line(
      points={{-34,9},{-34,40},{-11.5,40},{-11.5,46}},
      color={0,0,0},
      smooth=Smooth.None));
  connect(volumeSH.mbOut, mbOut[2])
    annotation (Line(
      points={{50,9},{50,40},{9.5,40},{9.5,46}},
      color={0,0,0},
      smooth=Smooth.None));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false,extent={{-100,-20},
            {80,40}}),  graphics),Icon(coordinateSystem(extent={{-100,-20},{80,40}},
                   preserveAspectRatio=true),
                                     graphics={
                               Bitmap(extent={{-102,108},{80,-86}}, fileName="modelica://ThermoCycle/Resources/Images/MB/Eva_D_wf.png")}));
end EvaDry;
