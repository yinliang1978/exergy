...
model DP_pT "Lumped, 3-terms pressure drop model, pT formulation"
...
equation
  ...
  p_fl = InFlow.p;
  T_fl = smooth(1,if Mdot >= 0 then OutFlow.T_outflow else InFlow.T_outflow);
  d_fl = Medium.density_pTX(p_fl,T_fl,if Mdot >= 0 then OutFlow.Xi_outflow else InFlow.Xi_outflow);
...
// Boundary conditions
  Mdot = InFlow.m_flow;
  InFlow.T_outflow = inStream(OutFlow.T_outflow);
  inStream(InFlow.T_outflow) = OutFlow.T_outflow;
