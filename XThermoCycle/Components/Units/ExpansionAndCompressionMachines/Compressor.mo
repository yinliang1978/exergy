within Exergy.XThermoCycle.Components.Units.ExpansionAndCompressionMachines;
...
Modelica.Mechanics.Rotational.Interfaces.Flange_b flange_elc "Flange of shaft"
  ...
  ThermoCycle.Interfaces.Fluid.FlangeA InFlow(redeclare package Medium =
            ...
            -78,68},{-58,88}}), iconTransformation(extent={{-78,68},{
            -58,88}})));
  ThermoCycle.Interfaces.Fluid.FlangeB OutFlow(redeclare package Medium =
               Medium) annotation (Placement(transformation(extent={{
            76,-50},{96,-30}}), iconTransformation(extent={{76,-50},{
            96,-30}})));
...
public
      outer Exergy.Utilities.RefEnv refEnv "reference enviroment properties";
        Exergy.Utilities.ViewObjectNoEff viewObject(nEnergy={2,0,1,0});
          Exergy.Utilities.ViewPort viewOutput
    annotation (Placement(transformation(extent={{86,88},{106,108}})));
   Real hI1=actualStream(InFlow.h_outflow);
   Real eI1=InFlow.m_flow*hI1;
   Real sI1=Medium.specificEntropy(Medium.setState_ph(InFlow.p,hI1));
   Real exI1= InFlow.m_flow*(hI1-refEnv.T*sI1);

   Real hO1=actualStream(OutFlow.h_outflow);
   Real eO1=OutFlow.m_flow*hO1;
   Real sO1=Medium.specificEntropy(Medium.setState_ph(OutFlow.p,hO1));
   Real exO1= OutFlow.m_flow*(hO1-refEnv.T*sO1);
...
equation
  ...
  viewObject.h[2].Ex_flow=  exO1;
   ...
   viewObject.w[1].E_flow =   W_dot;
  viewObject.w[1].Ex_flow=  W_dot;
