within Exergy.XThermoCycle.Components.HeatFlow.Walls;
model MetalWall
...
parameter Boolean steadystate_T_wall=true
  ...
  ThermoCycle.Interfaces.HeatTransfer.ThermalPort Wall_int(N=N, T(
        start=Tstart_wall)) annotation (Placement(transformation(
          extent={{-30,20},{30,40}}), iconTransformation(extent={{-30,
            20},{30,40}})));
  ThermoCycle.Interfaces.HeatTransfer.ThermalPort Wall_ext(N=N, T(
        start=Tstart_wall)) annotation (Placement(transformation(
          extent={{-30,-20},{30,0}}), iconTransformation(extent={{-32,
            -40},{28,-20}})));
/* METAL WALL */
